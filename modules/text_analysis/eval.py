...
for num, i in enumerate(sample):
    ...
    # """phase: grammar punctuation and spelling"""
    # note output "original_text"
    # note output "issue_indeces"
    # note output "corrected_text"
    # note output "correct_indeces"
    # note output "edit
    # note output "critique"
    # grammar_correction = analyze_text(i) #<------ review (dictionary) i -> original

    # #format of response: 1. original text, 2. Index of words that have issue
    # print("Original Text:", grammar_correction["original_text"])
    # print(f"\nissue_indeces:")
    # print(grammar_correction["issue_indeces"])
    # print()
    # print("Corrected Text:", grammar_correction["corrected_text"])
    # print()
    # print("Edits got:")
    # print(grammar_correction["edit"])
    # print()
    # print("Diffferences:")
    # # print(grammar_correction["diff_suggestions"])
    # for diff in grammar_correction["diff_suggestions"]:
    #     # if diff["category"] == "OTHER":
    #     print("-"*10)
    #     print(f"Category: {diff['category']}")
    #     print(f"Critique: {diff['critique']}")
    #     print(f"Original: {diff['original']}")
    #     print(f"Corrected: {diff['corrected']}")
    
    # """phase: readability analysis"""
    # scores, complex_words = readability_score(i) #<------ review (2 dictionaries) dic 1 contains - scores, dic 2 contains - complex words i-> original
    # #format of response: 1. flesch reading ease score, 2. text standard grade, 3. reading time, 4. complex word, its index, its frequency of usage in real word, suggestion word for replacement
    # print(f"\nReadability Score:")
    # for i in scores:
    #     print(f"{i} - {scores[i]}")
    ...
    # tone = detect_tone(i) #<------ review (dictionary) i-> original
    # #format of response: 1. tone of the text, 2. a random example of current tone, 3. example of converting current tone into other tones
    ...
    # word_analysis = get_word_analysis(i) #<------ review (dictionary) i->original
    # #format of response: 1. word count, 2. keywords, 3. phrases, 4. syllable count, 5. Lexicon count, 6. sentence count, 7. character count, 8. average words per sentence, 9. unique words, 10. type-token ratio (ttr = unique_word_count / word_count) -> a higher score means richer vocabulary
    ...
    # topic = get_topic(i) #<------ review (dictionary) i->corrected
    # #format of response: 1. topics list, 2. confidence score of each topic

    # for key, value in topic.items():
    #     print(f"{key}    ->    {(value * 100):.2f} %")

    # """phase: entity recognition"""
    # entities = entity_recognition(i)  #<------ review (list) i-> original
    # # format of response: 1. list of entities in text, 2. label of that entity
    # if len(entities) != 0:
